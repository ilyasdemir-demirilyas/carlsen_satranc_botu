...
'''
Burada alfa-beta budama yöntemiyle kullanıcının istediği derinlikte arama yapılır ve 
bu arama sonucu oluşan en iyi sonuç hamle olarak oynanır .
Oyun insan ile algoritma arasında oynancak şekilde tasarlanmıştır .
beyaz bilgisayar siyah ise kullanıcı olarak belirlenmiştir .
'''
...
def player(board):
	while 1==1:
		try:
			...
			board.push_san(move);display(board)
			break
		except ValueError:
